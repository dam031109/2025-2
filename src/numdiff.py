...
def run_case(f, df, a, tag, out_dir_data="data", out_dir_figs="figs"):
    ...
    for h in hs:
        ...
        rows.append([h, s1, s2, s3, true_val,
                     abs(s1-true_val), abs(s2-true_val), abs(s3-true_val)])

    cols = ["h","approx1","approx2","approx3","true","err1","err2","err3"]
    T = pd.DataFrame(rows, columns=cols)

    fmt = lambda x: float(f"{x:.10e}")  # 과학적 표기, 자릿수 통일
    for c in cols: T[c] = T[c].map(fmt)

    # 표 저장
    T.to_csv(f"{out_dir_data}/{tag}_table.csv", index=False)
    ...
    }).to_csv(f"{out_dir_data}/{tag}_mins.csv", index=False)
    ...
    plt.savefig(f"{out_dir_figs}/{tag}_error_curve.png", dpi=300)
    plt.close()
